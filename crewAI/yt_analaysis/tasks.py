...
class YoutubeAutomationTasks():
    ...
    def manage_youtube_video_creation(self, agent, video_topic, video_details):
        return Task(
            description=dedent(f"""Oversee the YouTube prepration process including market research, title ideation, 
                ...
                Twitter: https://x.com/daniel_das07
            ...
            expected_output=dedent(f"""
                Dont Generate a Exactly as the report that is formatted like the example report provided to you earlier.Be creative and cachy to attract the viewrs in the description part.
                Modify the Description. Strictly donot use the same format be as creative as posible .The above Report is just a sample .Make sure the report contains 15 videos, 10 potential high CTRO titles, a YouTube video description, and an email newsletter update about the new video.
    ...
    def create_youtube_video_description(self, agent, video_topic, video_details):
        return Task(
            ...
            agent=agent,
            expected_output=dedent(f"""                   
            Dont follow the exact pattern given above in the Description feel free to improvise the description 
            A description that is neatly formatted and it should be more relevant to the given {video_details}
            and {video_topic} the description should follow the Example Description be creative, cachy and Informative 
            """),
        )
