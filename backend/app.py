...
def generate_facebook_post():
    try:
        ...
        if not content:
            return jsonify({'error': 'Content is required to generate a Facebook post'}), 400

        # Initialize Facebook content generation
        facebook_manager = FacebookManager()
        generated_content = facebook_manager.generate_content(content)  # Adjust based on your implementation

        return jsonify({'content': generated_content})
    except Exception as e:
        app.logger.error(f'Error in generate_facebook_post route: {str(e)}')
...
def post_facebook():
    try:
        ...
        # Initialize Facebook manager and post content
        facebook_manager = FacebookManager()
        ...
        app.logger.error(f'Error in post_facebook route: {str(e)}')
        return jsonify({'error': str(e)}), 500
